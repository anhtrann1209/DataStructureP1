...
class TreeSet:
    ...
    #method only works if height of the childrens are correct computed
    #must be performed from the bottom
    def _recalculate_node_height(self, node):
        node.height = 1 + max(self._node_height, node.right.height)
    ...
    def _add_recursive(self, r, v):
        ...
        if v < r.value:
            ...
            r.left.parent = r
            #check node height after add
            self._recalculate_node_height(r)
            #only way is the left is greater than right height
    ...
    def _rotate_left(self, x):
        #you handed root change parent relationship
        y = x.right
        t2 = y.left
