...
"SET DOES NOT NEED KEY BUT NEED HAS TO ACCESS ITEM QUICK"
...
class HashSet:
    ...
    def _init_table(self, new_capacity:int):
        ...
        #every bucket contain a double linked list
        for i in range(len(self.the_table)):
            self.the_table[i] = DoublyLinkedList()

    def _expand_table(self):
        #looping through each bucket and get it value
        values = [v for bucket in self.the_table for v in bucket]
        self._init_table(self.size * 2)
        for v in values:
            #add values to current Hashset(iter)
            self.add(v)
    ...
    def __iter__(self):
        return iter(self.the_table)
    
    def __str__(self):
        #setting up printing as value in a set with each bucket [] as double linked list
        result = "{"
        for x in self.the_table:
            result += str(x) + ", "
        #take out the last comma in the list when printed
    ...
    def add(self, v)-> bool: 
        #pass in the value to compress it into a bucket
    ...
    def discard(self, v): # removes v from the set
        index = self._hash_and_compress(v)
        bucket = self.the_table[index]
        if v in bucket:
            bucket.remove_value(v)
